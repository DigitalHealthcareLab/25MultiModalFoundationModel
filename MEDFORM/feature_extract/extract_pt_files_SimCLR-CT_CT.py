...
# 특징 추출 및 저장
with torch.no_grad():
    for i, (images, sample_id) in enumerate(dataloader):
        ...
        for idx in range(images.size(2)):
            ...
            print(image.shape)
            # image = image.repeat(1, 3, 1, 1)  # 3채널로 확장
            feature = model(image)  # 특징 추출
            features[idx] = feature.flatten()  # 평탄화 후 저장

        # 파일 경로 및 저장
        file_path = f'/home/alleun1110/Digital_bio/TANGLE_renew/data/NSCLC_Radiogenomics_fixed/pt_files_SimCLR_CT_pretrained_representation_learning/{sample_id[0]}.pt'
        ...
        # dataset = {
        #     'case_id': sample_id[0],
        #     'slide_id': sample_id[0],
        #     'label': label.item()
        # }
#         df = pd.concat([df, pd.DataFrame([dataset])], ignore_index=True)

# # 최종 DataFrame을 CSV로 저장
# df.to_csv('/home/alleun1110/Digital_bio/CLAM/dataset_CT_pretrained_unsupervised.csv', index=False)
