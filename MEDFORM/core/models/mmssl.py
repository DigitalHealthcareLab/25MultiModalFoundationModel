...
class MMSSL(nn.Module):
    def __init__(self, config, n_tokens_rna): #config : 모델에 대한 하이퍼파라미터나 설정이 담긴 딕셔너리, n_tokens_rna : RNA 시퀀스에서 사용할 토큰 수
...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.blocks=nn.Sequential(
            ...
            nn.Linear(in_features=hidden_dim, out_features=self.output_dim), # hidden_layer에서 output_dim으로 크기를 변환하는 역할 (그냥 선형 layer임)
        )
        

    def build_block(self, in_dim, out_dim): # 일일이 정의해서 작성해줘도 되는데, 그냥 block 단위를 하나로 처리한 거임 (Linear, LayerNorm, ReLU, DropOut을 하나 세트로)
...
class ProjHead(nn.Module): # Projection head : 선형 계층 하나로, input_dimension을 output_dimension으로 그냥 변환시켜주는 역할임.
